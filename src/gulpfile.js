...
gulp.task('sass', () => {
    return gulp
        .src(['client/styles/**/*.sass', 'client/styles/**/*.scss'])
        .pipe(
            sass({
...
gulp.task('css-clean', () => {
    return gulp.src('public/styles/').pipe(clean());
});
...
// CSS copy
gulp.task('css-copy', () => {
    return gulp.src('client/styles/**/*.css').pipe(gulp.dest('public/styles'));
});

// CSS copy for production
gulp.task('css-copy-prod', () => {
    return gulp.src('client/styles/**/*.css').pipe(gulp.dest('tmp/styles'));
});

// Assets copy
gulp.task('assets-copy', () => {
    return gulp.src('client/assets/**/*.*').pipe(gulp.dest('public/assets/'));
});
...
gulp.task('js-clean', () => {
    return gulp.src('public/scripts/').pipe(clean());
});
...
gulp.task('js-copy-prod', () => {
    return gulp.src('./client/scripts/**/*.js').pipe(gulp.dest('tmp/scripts'));
});
...
const loadData = () => {
    ...
    try {
        const yamlData = yaml.load(
            fs.readFileSync('./client/data.yaml', {
                encoding: 'utf-8',
            })
        );
...
gulp.task('pug-link-images', () => {
    const hashes = JSON.parse(
        fs.readFileSync('./tmp/assets-hashes.json', {
            encoding: 'utf-8',
        })
    );
    ...
    _(hashes).each((val, key) => {
        gulpObj = gulpObj.pipe(
            replace(key, `${process.env.CACHE_HOST || ''}${val}`)
        );
    });
...
gulp.task('concat-css', () => {
    return gulp
        .src(pipeline.cssFilesToInject.map((path) => `tmp/styles/${path}`))
        .pipe(concat('styles.css'))
        .pipe(gulp.dest('./tmp/'));
});
gulp.task('concat-css-dev', () => {
    return gulp
        .src(pipeline.cssFilesToInject.map((path) => `public/styles/${path}`))
        .pipe(concat('styles.css'))
        .pipe(gulp.dest('./tmp/'));
});
gulp.task('concat-email-css', () => {
    return gulp
        .src(['tmp/styles/emails/*.css'])
        .pipe(concat('email-styles.css'))
        .pipe(gulp.dest('./tmp/'));
});
gulp.task('concat-email-css-dev', () => {
    return gulp
        .src(['public/styles/emails/*.css'])
        .pipe(concat('email-styles.css'))
        .pipe(gulp.dest('./tmp/'));
});
...
gulp.task('concat-js', () => {
    return gulp
        .src(pipeline.jsFilesToInject.map((path) => `tmp/scripts/${path}`))
...
const copyNpm = (location, type, prod, filename) => {
    ...
    if (filename) {
        return gulp
            .src(`${process.cwd()}/node_modules/` + location.replace(/^\//, ''))
            .pipe(
                rename((path) => {
...
gulp.task(
    ...
    gulp.series(
        ...
        () => {
            ...
            jsWatch.on(
                'add',
                gulp.series('make-js', 'grunt-sails-linker:devJs', 'pug-static')
            );
...
gulp.task(
    ...
    gulp.series(
        ...
        gulp.parallel('concat-js', 'concat-css', 'concat-email-css'),
        gulp.parallel('grunt-bushcaster:assets'),
        gulp.parallel(
            // Minify css + js
            'minify-css',
            'minify-js'
