...
const makeUserController = (User) => {
    if (typeof User === 'undefined') {
        throw new Error(
            'makeUserController must be called with the user model specified'
        );
    }
    ...
    controller.changePassword = (req, res, next) => {
        ...
        if (!req.body.password || !req.body.new_password) {
            return next(
                new APIError(401, 'The old and the new password must be set.')
            );
        }
        ...
        return (
            req.user
                ...
                .then((user) =>
                    user.checkPassword(req.body.password).then((result) => {
    ...
    controller.createUser = (req, res, next) => {
        ...
        return new User({
            ...
            .then((user) => {
                ...
                return new User(_.omit(req.body, 'password')).setPassword(
                    req.body.password
                );
            })
            .then((user) => user.save())
    ...
    controller.makeFirstUser = (req, res, next) => {
        if (!process.env.FIRST_USER_TOKEN) return res.sendStatus(501);
        ...
        if (
            ...
            throw new APIError(
                403,
                "A first user token must be provided or it didn't match the server one"
            );
        }
