...
const makeSessionController = (User, Session) => {
    if (typeof User === 'undefined' || typeof Session === 'undefined') {
        throw new Error(
            'makeSessionController must be called with the user and session model specified'
        );
    }
    ...
    controller.createSession = (req, res, next) => {
        // Sanity checks
        if (!req.body.email)
            throw new APIError(400, 'Email must be set to log in');
        if (!req.body.password)
            throw new APIError(400, 'Password must be set to log in');
